@extends('../layouts/admin-side-navbar')

<head>
    @vite('resources/css/app.css')
</head>
{{-- @include('admin.admin-side-navbar'); --}}
@section('content')
    <div class="overflow-x-auto overflow-y-hidden rounded-lg border border-gray-200 w-full shadow-md m-5">
        <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
            <thead class="bg-gray-50">
                <tr>
                    <th scope="col" class="px-6 py-4 font-medium text-gray-900">Product Images</th>
                    <th scope="col" class="px-6 py-4 font-medium text-gray-900">User name</th>
                    <th scope="col" class="px-6 py-4 font-medium text-gray-900">User Email</th>
                    <th scope="col" class="px-6 py-4 font-medium text-gray-900">Order quantity</th>
                    <th scope="col" class="px-6 py-4 font-medium text-gray-900">Order Price</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100 border-t border-gray-100">
                @foreach ($data as $d)
                    <tr class="hover:bg-zinc-200">
                        <th class="flex gap-3 px-6 py-4 font-normal text-gray-900">
                            <div class="relative h-10 w-10">
                                <img class="h-full w-full rounded-full object-cover object-center"
                                    src="{{URL::to('/')}}/product_pic/{{ $d['product_pic'] }}" alt="" />
                            </div>
                            <div class="text-sm">
                                <div class="font-medium text-gray-700">{{ $d['product_name'] }}</div>
                                <div class="text-gray-400">{{ $d['product_brand'] }}</div>
                            </div>
                        </th>
                        <td class="px-6 py-4">
                            <span>{{ $d['user_name'] }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <span>{{ $d['user_email'] }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <span>{{ $d['order_quantity'] }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <span>{{ $d['order_price'] }}</span>
                        </td>
                        
                       
                        {{-- <td class="px-6 py-4">{{$d['user_role']}}</td> --}}

                        {{-- <td class="px-6 py-4">
                     <div class="flex justify-end gap-4">
                         <a x-data="{ tooltip: 'Delete' }" href="#">
                             <svg  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                 stroke-width="1.5" stroke="currentColor" class="h-6 w-6 stroke-red-600"
                                 x-tooltip="tooltip">
                                 <path stroke-linecap="round" stroke-linejoin="round"
                                     d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                             </svg>
                         </a>
                     </div>
                 </td> --}}
                        
                    </tr>
                @endforeach
            </tbody>
        </table>
    </div>
@endsection
